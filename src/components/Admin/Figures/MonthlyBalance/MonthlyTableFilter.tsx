...
export default function MonthlyTableFilter(props: TableFilterProps) {
  ...
  const monthsAgoOnClick = (month: number) => {
    const dateInput = dayjs().subtract(month, 'month');
      ...
      <div className={'flex flex-row-reverse gap-2 justify-end'}>
        {[...Array(9)].map((row, index) => (
          <Button
            key={index}
            variant="contained"
            onClick={() => monthsAgoOnClick(index)}
            sx={{ minnWidth: 130 }}
          >
            {index > 1 ? `${index} months ago` : null}
            {index === 1 ? 'Last month' : null}
            {index === 0 ? 'This month' : null}
          </Button>
        ))}
      </div>
    </div>
